...
public class SessionUtils {
     ...
     */
    public static void setUser(HttpServletRequest request, Customer user) {
     ...
     * Lấy user từ session
     * @param request HttpServletRequest
     * @return Customer object hoặc null nếu chưa đăng nhập
     */
    public static Customer getUser(HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        if (session != null) {
            return (Customer) session.getAttribute(Constants.SESSION_ACCOUNT);
        }
        return null;
    }
    ...
    public static boolean isAdmin(HttpServletRequest request) {
        Customer user = getUser(request);
        return user != null && user.getId() == Constants.ROLE_ADMIN;
    }
    ...
    public static boolean isUser(HttpServletRequest request) {
        Customer user = getUser(request);
        return user != null && user.getId() == Constants.ROLE_USER;
    }
