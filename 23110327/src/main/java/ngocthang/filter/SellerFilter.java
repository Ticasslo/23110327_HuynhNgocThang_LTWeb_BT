...
public class SellerFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Kiểm tra quyền Seller (chỉ Seller mới được truy cập)
        if (!SessionUtils.isSeller(req)) {
            // Không có quyền Seller - chuyển hướng về waiting để phân tích role
            resp.sendRedirect(req.getContextPath() + "/waiting");
            return;
        }
        
        // Có quyền Seller - tiếp tục xử lý request
